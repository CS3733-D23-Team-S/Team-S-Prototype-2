...
public class LoaderDAO implements LoaderDAOI {
	...
	public void loadCSV(String importPath, char dataType) {
		try {
			...
			switch (dataType) {
				case 'n':
					System.out.println(NodeDaoImpl.getInstance().getName());
					dropTable += NodeDaoImpl.getInstance().getName() + " CASCADE";
					stmt.executeUpdate(dropTable);
					NodeDaoImpl.getInstance().initTable(NodeDaoImpl.getInstance().getName());
					NodeDaoImpl.getInstance().loadToRemote(importPath);
					break;
				case 'e':
					dropTable += EdgeDaoImpl.getInstance().getName() + " CASCADE";
					stmt.executeUpdate(dropTable);
					EdgeDaoImpl.getInstance().initTable(EdgeDaoImpl.getInstance().getName());
					EdgeDaoImpl.getInstance().loadToRemote(importPath);
					break;
				case 'l':
					dropTable += LocationDoaImpl.getInstance().getName() + " CASCADE";
					stmt.executeUpdate(dropTable);
					LocationDoaImpl.getInstance().initTable(LocationDoaImpl.getInstance().getName());
					LocationDoaImpl.getInstance().loadToRemote(importPath);
					break;
				case 'm':
					dropTable += LocationDoaImpl.getInstance().getName() + " CASCADE";
					stmt.executeUpdate(dropTable);
					MoveDaoImpl.getInstance().initTable(MoveDaoImpl.getInstance().getName());
	...
	public void exportCSVs(String exportPath) {
		...
		System.out.println("Exported all of the csv file to " + exportPath);
	}
